...
const ProfileTab = ({ user, setUser, handleLogout }) => {
    ...
    const MainProfileView = () => (
        <div className="w-full max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
            <div className="flex justify-between items-center p-4 border-b border-gray-200">
                <div className="w-6"></div> {/* Spacer */}
                <h1 className="text-xl font-bold text-gray-800">My Profile</h1>
                <button onClick={() => setView('edit')} className="text-gray-500 hover:text-orange-500 transition-colors">
                    <SettingsIcon />
                </button>
            </div>
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-200">
                <button onClick={handleLogout} className="w-full flex items-center justify-center p-3 text-gray-600 font-semibold bg-white rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                    <LogoutIcon />
                    Log Out
                </button>
            </div>
...
const EditProfileView = ({ user, setUser, goBack }) => {
    ...
    const FormInput = ({ label, value, onChange, type = 'text', placeholder, required = true, disabled = false }) => (
        <div>
            ...
            <input 
                ...
                required={required}
                disabled={disabled}
                className="w-full p-3 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/50 disabled:bg-gray-100"
            />
        </div>
    );
    ...
    return (
        <div className="w-full max-w-md mx-auto p-4">
            <div className="flex items-center mb-6">
                <button onClick={goBack} className="mr-4 text-gray-600 hover:text-orange-500">
                    <BackIcon />
                </button>
            ...
            <div className="space-y-8">
                ...
                <form onSubmit={handlePasswordChange} className="space-y-4 p-6 bg-white rounded-lg shadow-sm border border-gray-200">
                    ...
                    <button type="submit" disabled={isChangingPassword} className="w-full bg-gray-700 text-white font-semibold py-3 rounded-lg shadow-sm hover:bg-gray-800 disabled:opacity-50 transition-colors">
...
const AppointmentsList = ({ appointments, isLoading }) => {
    ...
    if (isLoading) return <p className="text-center text-gray-500 p-4">Loading appointments...</p>;

    return (
        <div className="p-4 space-y-3">
            <h3 className="text-lg font-semibold text-gray-700 px-2">My Appointments</h3>
            {appointments.length > 0 ? appointments.map(appt => (
                <div key={appt._id} className="p-3 bg-white rounded-lg border border-gray-200 flex justify-between items-center hover:bg-gray-50 transition-colors">
                    <div>
                ...
                <p className="text-center text-gray-500 p-4">No appointments scheduled.</p>
            )}
        </div>
    );
};
