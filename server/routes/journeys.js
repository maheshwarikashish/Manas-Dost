...
// --- [DEFINITIVE FIX] --- 
// POST a new custom journey using a robust, text-based approach
router.post('/custom', auth, async (req, res) => {
    ...
    try {
        // 1. SIMPLIFIED PROMPT: Ask for a simple, numbered list, not complex JSON.
        const prompt = `
        ...
        // 2. ROBUST PARSING: Split the response by lines and filter for the expected format.
        // This avoids fragile JSON.parse() and is highly resilient to extra text from the AI.
        ...
        // 3. BACKEND-CONTROLLED STRUCTURING: Create the JSON object on the server.
        const newJourney = {
            id: `custom-${Date.now()}`,
            ...
            tasks: tasks.slice(0, 7).map(task => ({
                task: task.replace(/^Day \d+: /i, '').trim(), // Clean the task string
                completed: false
            }))
        };
